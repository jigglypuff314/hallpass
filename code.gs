...
function onFormSubmit(e) {
  ...
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const submittedSheet = e.range.getSheet(); // Get the sheet where the submission occurred

  // Check if the submission is on the main form sheet
  if (submittedSheet.getName() !== SHEET_NAME) {
    Logger.log(`onFormSubmit: Submission not on target sheet "${SHEET_NAME}". Ignoring.`);
    return; // Exit the function if it's not the main form sheet
  }
...
function onEndPassFormSubmit(e) {
  ...
  // Check if the submission is on the End Pass sheet
  if (submittedSheet.getName() !== END_PASS_SHEET_NAME) {
    Logger.log(`onEndPassFormSubmit: Submission not on target sheet "${END_PASS_SHEET_NAME}". Ignoring.`);
    return; // Exit the function if it's not the End Pass sheet
  }
...
function markExpiredPasses() {
 ...
 Logger.log(`Warning: markExpiredPasses - Activation timestamp missing for row ${originalRow}. Using submission time: ${startTime.toLocaleString()}.`);
      ...
      Logger.log(`markExpiredPasses: Checking pass for ${studentEmail} (row ${originalRow}). Activated: ${startTime.toLocaleString()}, Expires: ${expiryTime.toLocaleString()}. Current time: ${now.toLocaleString()}`);
...
function promoteWaitlist() {
 ...
 todayEnd.setHours(23, 59, 59, 999); // End of today
  ...
  for (const pass of waitlistedPasses) {
    ...
    const originalRow = pass.originalRow;
    const originalSubmissionTime = new Date(pass.data[0]);

    Logger.log(`promoteWaitlist - Considering Row ${originalRow} for promotion. Email: ${studentEmail}, Room: ${roomNumber}, OriginalSubmission: ${originalSubmissionTime.toLocaleString()}`);
 ...
 pass.data.push("");
      ...
      // Update status and activation timestamp in memory for this pass's data
 ...
 Logger.log(`    - New Activation time (Col N): ${newActivationTime.toLocaleString()}`);
        ...
        Logger.log(`  - ERROR sending promotion email to ${studentEmail} (row ${originalRow}): ${emailError.message}`);
      }
      ...
      allData[originalRow - 1] = pass.data; // Use original row index (0-indexed)
